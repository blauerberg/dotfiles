...
check_badge "linux host badge"         "OSTYPE=linux-gnu"                           present "%K{blue}%F{white} 🐧 LINUX %f%k"
check_badge "host custom label"        "OSTYPE=linux-gnu; DOTFILES_HOST_BADGE=tux"  present "%K{blue}%F{white} tux %f%k"
check_badge "host badge disabled"      "OSTYPE=linux-gnu; DOTFILES_HOST_BADGE="     absent  "%K{blue}"
check_badge "container wins over host" "OSTYPE=linux-gnu; DEVCONTAINER=1"           absent  "%K{blue}"
